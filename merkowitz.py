...
start = pd.to_datetime('2018-12-12')
...
def optimize_portfolio(weights, returns):
	...
	bounds = tuple((min_allocation, max_allocation) for x in range(
		len(stocks)))  # the weights can be 1 at most: 1 when 100% of money is invested into a single stock
	optimum = optimization.minimize(fun=min_func_sharpe, x0=weights, args=returns, method='SLSQP', bounds=bounds,
...
def print_optimal_portfolio(optimum, returns):
	...
	for i in range(0,len(stocks)):
		print(" %6s : % 4.3f" % (stocks[i], optimum['x'][i]) )
